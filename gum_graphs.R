...
justspots <- data.frame(gumspots$SQ1, gumspots$SQ2, gumspots$SQ3, gumspots$SQ4, gumspots$SQ5, gumspots$SQ6, gumspots$SQ7, gumspots$SQ8, gumspots$SQ9, gumspots$SQ10, gumspots$SQ11, gumspots$SQ12, gumspots$SQ13, gumspots$SQ14, gumspots$SQ15, gumspots$SQ16)
stack <- melt(justspots)

#general gumspot graph
ggplot(gumspots, aes(Start)) + 
...
ggplot(gumspots[41:100,], aes(Start)) + 
  geom_line(aes(y=SQ.AVG, color = "Gumspots per Flag")) + 
  geom_line(aes(y=X5.BLK.SQAVG, color = "Smoothed"), size=1.3) + 
...
#R is famously difficult to create dual y-axes graphs with
...
#gumspots vs PCI
ggplot(gumspots[41:100,], aes(x=Per.Capita.Income, y=X5.BLK.SQAVG)) + 
